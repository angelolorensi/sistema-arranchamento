...
export class CadastrarEfetivoComponent implements OnInit {
  ...
  displayedColumns = ['graduacao','nomeDeGuerra','identidade','numero','cia','om',];

  constructor(private arranchamentoService: ArranchamentoService, private formBuilder: FormBuilder, private snackBar:MatSnackBar, private router: Router, private route: ActivatedRoute) {
  ...
  onSubmit() {
    this.arranchamentoService.salvarMilitar(this.form.value).subscribe(result =>  this.onSuccess(), error => this.onError);
  }
